...
#: Directories under ``csrc/shipvision/`` with no Python counterpart. ``core`` is the C++-only
#: foundation — the vendor aliases and the scratch allocators — and has nothing to mirror.
NATIVE_ONLY = {"core"}
...
class TestNativeLayout:
    ...
    def test_the_native_tree_mirrors_the_python_package(self) -> None:
        ...
        for path in sorted(NATIVE.iterdir()):
            if not path.is_dir() or path.name in NATIVE_ONLY:
                continue
            if not (PACKAGE / path.name / "__init__.py").is_file():
                offenders.append(str(path.relative_to(REPO)))

        assert not offenders, (
            "native directories with no Python family of the same name. A fused kernel is only "

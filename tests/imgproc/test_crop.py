...
def test_a_box_partly_outside_the_frame_is_clamped_not_dropped(ops, frame) -> None:
...
def test_a_box_past_the_far_edge_clamps_to_the_last_addressable_pixel(ops, frame) -> None:
...
def test_a_box_entirely_outside_the_frame_yields_a_blank_crop(ops, frame) -> None:
...
def test_a_zero_area_box_does_not_divide_by_zero(ops, frame) -> None:
...
def test_an_inside_out_box_is_treated_as_having_no_area(ops, frame) -> None:
...
def test_a_blank_crop_does_not_contaminate_its_neighbours(ops, frame) -> None:
...
def test_no_boxes_gives_an_empty_batch_not_an_error(ops, frame) -> None:
...
def test_a_crop_of_a_uniform_region_is_that_colour_in_rgb(ops) -> None:
...
def test_a_crop_follows_the_box_along_each_axis_independently(ops) -> None:
...
def test_the_output_extent_is_the_target_not_the_box(ops, frame) -> None:
...
def test_a_malformed_box_array_is_refused(ops, frame) -> None:
    with pytest.raises(DimensionMismatchError):
        ops.crop_batch(frame, np.zeros((4, 5), dtype=np.float32), TARGET)


def test_a_malformed_frame_is_refused(ops) -> None:
    with pytest.raises(DimensionMismatchError):
        ops.crop_batch(

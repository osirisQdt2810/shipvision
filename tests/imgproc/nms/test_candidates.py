...
def test_the_score_threshold_is_inclusive(ops) -> None:
...
def test_no_boxes_gives_no_indices(ops) -> None:
    keep = ops.nms(
...
def test_one_box_survives_itself(ops) -> None:
...
def test_survivors_come_back_in_descending_score_order(ops) -> None:
...
def test_a_tie_breaks_towards_the_lower_input_index(ops) -> None:
...
def test_none_applies_the_score_threshold_and_nothing_else(ops) -> None:
...
def test_every_documented_method_runs(ops) -> None:
...
def test_a_score_and_box_count_mismatch_is_refused(ops) -> None:
...
def test_an_unknown_method_is_refused_by_name(ops) -> None:
...
def test_an_out_of_range_iou_threshold_is_refused(ops) -> None:
...
def test_gauss_refuses_a_non_positive_sigma(ops) -> None:

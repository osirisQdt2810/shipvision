...
def test_scale_preserves_aspect_by_taking_the_smaller_ratio() -> None:
...
def test_the_resized_extent_rounds_half_up_not_to_even() -> None:
...
def test_an_odd_total_pad_puts_the_extra_pixel_at_the_bottom() -> None:
...
def test_the_horizontal_pad_splits_the_same_way() -> None:
...
def test_the_pad_split_is_visible_in_the_tensor_itself() -> None:
...
def test_the_channel_order_of_the_output_is_rgb() -> None:
...
def test_resize_centres_follow_the_half_pixel_convention() -> None:
...
def test_letterbox_upsampling_matches_the_hand_computed_taps() -> None:
...
def test_crop_centres_offset_the_subregion_before_the_half_pixel_shift() -> None:
...
def test_invert_boxes_recovers_a_geometrically_mapped_box_within_a_pixel(
    ...
    mapped[:, 1::2] = (
        source[:, 1::2] * geometry.resized_height / geometry.source_height + geometry.pad_top
    )
...
def test_invert_boxes_clips_to_the_source_extent() -> None:
...
def test_invert_boxes_accepts_an_empty_frame() -> None:
    geometry = LetterboxGeometry.plan(*EVEN_PAD)
...
def test_invert_boxes_refuses_a_bare_box() -> None:
...
def test_invert_points_preserves_shape(shape: tuple[int, ...]) -> None:
...
def test_invert_points_carries_a_keypoint_confidence_through_untouched() -> None:
...
def test_a_zero_target_is_refused_at_planning_time() -> None:
    with pytest.raises(ConfigurationError):
        LetterboxGeometry.plan((100, 100), (0, 640))


def test_an_empty_source_is_refused_at_planning_time() -> None:
    with pytest.raises(DimensionMismatchError):
        LetterboxGeometry.plan((0, 100), (640, 640))


def test_a_geometry_cannot_be_mutated_after_the_fact() -> None:

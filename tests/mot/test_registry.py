...
under ``trackers/`` plus a decorator, and that nothing anywhere keeps a second, hand-maintained
...
TRACKERS_DIR = Path(__file__).resolve().parents[2] / "shipvision" / "mot" / "trackers"
...
def core_packages() -> list[str]:
    return sorted(m.name for m in pkgutil.iter_modules([str(TRACKERS_DIR)]) if m.ispkg)


class TestTheRegistryAndTheDirectoryAgree:
    """``trackers/`` and ``TRACKERS.names()`` are two views of one fact, so they cannot disagree.
    ...
    def test_every_core_package_is_registered(self) -> None:
        ...
        assert not missing, (
            f"{missing} is a package under trackers/ but is not in TRACKERS. Its __init__.py "
            f"must import its tracker module, and trackers/__init__.py must import the package"
        )
    ...
    def test_every_registered_tracker_has_a_package(self) -> None:
        ...
        assert not stray, (
            f"{stray} is registered but has no package under trackers/. A tracker defined "
            f"outside trackers/ is one nobody will find when they go looking for it"
        )
    ...
    def test_a_tracker_resolves_to_a_class_in_its_own_package(self, name: str) -> None:
        ...
        assert cls.__module__ == f"shipvision.mot.trackers.{name}.tracker", (
...
class TestAddingATrackerIsAPackagePlusADecorator:
    ...
    a test leaks into every test that runs after it, and ``tests/mot/test_contract.py``
...
class TestTheRegistryModuleStaysALeaf:
    ...
    That is only true while the module has no runtime import from ``shipvision.mot``, so
    the property is checked rather than trusted.
    """

    def test_it_has_no_runtime_import_from_the_mot_package(self) -> None:
        path = TRACKERS_DIR.parent / "registry.py"
        ...
        offenders = [
            ...
            if isinstance(node, ast.ImportFrom)
            and (node.module or "").startswith("shipvision.mot")
            and node not in guarded
        ]

        assert not offenders, (
            "shipvision/mot/registry.py imports from its own package at runtime; it is "
    ...
    def test_one_registry_object_is_shared_by_every_path_that_exposes_it(self) -> None:
        ...
        import shipvision
        import shipvision.mot.base as base
        import shipvision.mot.registry as leaf
...
class TestRegistrationHappensOnImport:
    ...
    that algorithm. ``import shipvision.mot.trackers.sort`` runs the parent ``__init__`` files
    first, and ``mot/__init__.py`` imports ``trackers``, which imports every one. The split
    ...
    def test_importing_the_family_registers_every_tracker(self) -> None:
        assert self.names_after("import shipvision.mot") == sorted(PUBLISHED)
    ...
    def test_importing_a_single_algorithm_package_is_enough(self) -> None:
        ...
        assert "sort" in self.names_after("import shipvision.mot.trackers.sort")
    ...
    def test_reaching_only_the_leaf_registry_module_still_registers_them(self) -> None:
        ...
        because making ``mot/__init__.py`` lazy would break ``TRACKERS.build`` for
        exactly this caller and for nobody else, which is a hard bug to find."""
        assert self.names_after("import shipvision.mot.registry") == sorted(PUBLISHED)
...
class TestTheDocumentedEntryPointStillWorks:
    ...
    def test_build_by_name_returns_a_working_tracker(self) -> None:
        module = importlib.import_module("shipvision.mot")

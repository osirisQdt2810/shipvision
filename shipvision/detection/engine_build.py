...
def build_engine(
    ...
    *,
    ...
    if onnx.stat().st_size == 0:
        # Before the runtime is required, like the other refusals here: an empty file is a
        # broken export or an interrupted download, and it is the same fault on a machine with
        # tensorrt and one without. Left to the parser it surfaced as `BackendUnavailableError`
        # on a laptop — "install tensorrt" — for a file no version of tensorrt could read.
        raise ModelLoadError(f"empty ONNX at {onnx}: a broken export, not a model")

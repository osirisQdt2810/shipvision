"""Compatibility shim: ``shipvision.mtmc.matrix.base`` before the repackaging.
...
BaseMatrixBuilder`` kept working after the move while ``import
shipvision.mtmc.matrix.base`` raised ``ModuleNotFoundError``, and both were public.

Aliases, never subclasses, so ``isinstance`` and ``is`` cannot start disagreeing depending on
which spelling a caller reached for.
"""

from shipvision.mtmc.matrix import MATRIX_BUILDERS, NEVER_MERGE, BaseMatrixBuilder

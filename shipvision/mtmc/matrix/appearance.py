...
shipvision.mtmc.matrix.appearance`` raised ``ModuleNotFoundError``, and both were public.

Aliases, never subclasses, so ``isinstance`` and ``is`` cannot start disagreeing depending on
which spelling a caller reached for.
"""

from shipvision.mtmc.matrix import AppearanceMatrixBuilder, stack_embeddings

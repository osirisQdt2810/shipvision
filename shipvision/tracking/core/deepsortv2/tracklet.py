...
def new_pool(
    *, max_age: int, min_hits: int, embedding_momentum: float, re_update: bool
    ...
    :func:`~shipvision.tracking.core.deepsortv2.utils.dynamic_momentum` computes a per-detection
    rate that rises towards the upper bound as a detection gets less confident or more crowded,
    and hands it to :meth:`~shipvision.tracking.pool.TrackPool.apply_matches` per frame. Passing
    ...
    Args:
        ...
        re_update: enable ORU. See :mod:`shipvision.tracking.core.ocsort.tracklet`, which

...
* A **dynamic appearance** EMA, whose rate depends on how confident and how isolated the
  detection was. See :mod:`shipvision.mot.association.appearance`.
...
The four costs themselves live in :mod:`shipvision.mot.trackers.deepsortv2.utils`, so that
this file is the ordering and nothing else.
...
from shipvision.mot.association import associate_subset, cascade_associate, pairwise_appearance
...
from shipvision.mot.trackers.deepsortv2.tracklet import new_pool
from shipvision.mot.trackers.deepsortv2.utils import (
    dynamic_momentum,
    off_border,
    stage_a_cost,
    stage_b_cost,
...
class DeepSortV2Tracker(BaseTracker):
    ...
    def _stage_a_cost(
        ...
        return stage_a_cost(
            ...
            giou_gate=self._giou_gate,
            gating_distances=self._pool.gating_distance(boxes, rows),
        )
    ...
    def _stage_c_cost(
        ...
        boxes = np.stack([kept[c].box for c in columns])
        return stage_c_cost(self._pool.observed_boxes()[rows], boxes)

    def _stage_d_cost(
        self, rows: list[int], columns: list[int], kept: list[Detection]
    ) -> np.ndarray:
        boxes = np.stack([kept[c].box for c in columns])
        return stage_d_cost(self._pool.boxes()[rows], boxes)

    def _appearance(
        self, rows: list[int], columns: list[int], kept: list[Detection]
    ) -> np.ndarray | None:
        """``(len(rows), len(columns))`` cosine distance, or ``None`` if either side lacks one."""
        return pairwise_appearance(
            self._pool.embeddings(), rows, [kept[c].embedding for c in columns]
        )
    ...
    def _momentum(self, kept: list[Detection]) -> np.ndarray | None:
        if not self._dynamic_appearance:
            return None
        return dynamic_momentum(kept, bounds=self._momentum_bounds)
    ...
    def _recoverable(self, rows: list[int], detections: Detections) -> list[int]:
        ...
        last observation was not against the frame edge. The two filters are separate because
        only the second one is geometry: the state filter needs the pool and stays here, the
        border rule is pure arithmetic and lives in
        :func:`~shipvision.mot.trackers.deepsortv2.utils.off_border` where it can be tested
        against hand-computed margins.
        """
        eligible = [
            row
            for row in rows
        ...
        # `not eligible` short-circuits before touching the pool. The extraction dropped that
        # clause, so an empty candidate list still built the whole `observed_boxes()` array —
        # and stage C runs once per frame per camera, on the path that is already the one
        # under measurement. Restored, and named here because it looks like a redundant guard
        # right up until you notice what `observed_boxes()` costs.
        if not self._skip_border_recovery or not eligible:
            return eligible
        return off_border(
            self._pool.observed_boxes(),
            eligible,
            height=detections.height,
            width=detections.width,
            border_fraction=self._border_fraction,
        )
...
# it, because deepsortv2 is *one* tracker with two implementations and splitting them by language
# is splitting them by the least interesting thing about them. The readable class above is the
# specification; the parity tests assert the two agree.
#
# What is *not* here is the machinery neither implementation owns: finding `shipvision._C`,
# refusing with a build command when it is absent, and marshalling numpy across the binding.
# That lives in `mot/backends/base.py`, because five copies of it would be five places to
# disagree about what an empty detection set looks like.


@TRACKERS.register("deepsortv2", backend=NATIVE)
class NativeDeepSortV2Tracker(NativeTracker):
    """DeepSORTv2's four-stage cascade in C++. See
    :class:`~shipvision.mot.trackers.deepsortv2.tracker.DeepSortV2Tracker` for the algorithm.
    ...
    *vector*, which never crosses. The cosine distances the cascade reads are built here, and
    ...
    :func:`~shipvision.mot.trackers.deepsortv2.utils.dynamic_momentum`, the same function the
    ...
    def __init__(
        ...
        *,
        ...
        """
        native = require_extension("DeepSortV2Tracker")

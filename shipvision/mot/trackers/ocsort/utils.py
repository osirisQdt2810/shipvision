...
The primitives are shared (:mod:`shipvision.mot.association`,
:mod:`shipvision.mot.motion`). What is OC-SORT's own — and what these two functions say
...
from shipvision.mot.association import direction_cost, gate_cost, iou_cost
    ...
    *,
    ...
    Args:
        ...
        headings: ``(n, 2)`` unit vectors from
            :meth:`~shipvision.mot.pool.TrackPool.directions`, or ``None`` when the
...
def recovery_cost(observed_boxes: np.ndarray, detection_boxes: np.ndarray) -> np.ndarray:
    ...
    Args:
        observed_boxes: ``(n, 4)`` xyxy last observations, from
            :meth:`~shipvision.mot.pool.TrackPool.observed_boxes`. Not predictions.

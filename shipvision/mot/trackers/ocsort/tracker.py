...
produced, and run the filter through them instead. Lives in
:mod:`shipvision.mot.trackers.ocsort.tracklet`, because it is a property of the state.
...
association (that is what :class:`~shipvision.mot.trackers.bytetrack.tracker.ByteTrackTracker`
is for, and combining the two is a fourth tracker, not a flag), and the "OC-SORT + appearance"
variants from later papers.
"""
...
class OcSortTracker(BaseTracker):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            new_pool(max_age=max_age, min_hits=min_hits, delta_t=delta_t, re_update=re_update)
        )
    ...
    def _primary_cost(
        ...
        # directions() and observed_boxes() are only read when the momentum term is on, so a
        # tracker with momentum_weight=0 pays nothing for the ring it still maintains.
        momentum_on = self._momentum_weight > 0.0
        return primary_cost(
            self._pool.boxes()[rows],
            boxes,
            headings=self._pool.directions(self._delta_t)[rows] if momentum_on else None,
            origins=self._pool.observed_boxes()[rows] if momentum_on else None,
    ...
    def _recovery_cost(
        ...
        boxes = np.stack([kept[c].box for c in columns])
        return recovery_cost(self._pool.observed_boxes()[rows], boxes)
...
# it: ocsort is one tracker with two implementations, and splitting them by language splits
...
class NativeOcSortTracker(NativeTracker):
    """OC-SORT with its two association stages and its re-update in C++. See
    :class:`~shipvision.mot.trackers.ocsort.tracker.OcSortTracker` for the algorithm.
    ...
    def __init__(
        ...
        *,
        ...
        """
        native = require_extension("OcSortTracker")

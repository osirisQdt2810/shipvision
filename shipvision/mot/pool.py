...
from shipvision.mot.base import next_track_id
from shipvision.mot.motion.kalman import KalmanFilter
...
class TrackPool:
    ...
    def max_age(self) -> int:
        """Frames a track survives unmatched. Read by trackers for their `describe()`."""
        return self._max_age

    @property
    def embedding_momentum(self) -> float:
        """The configured EMA retention, so a tracker deriving a per-detection one from
        confidence has the base to scale from rather than a second copy of the constant."""

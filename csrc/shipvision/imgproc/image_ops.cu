...
namespace shipvision {
    namespace {
        ...
        /// One NV12 pixel as RGB, BT.601 limited range, nearest chroma. Convention 1 and 2.
        ///
        /// Nearest chroma is a shift, not a filter: `>> 1` on both coordinates. The alternative
        /// — bilinear on the chroma plane — costs four more loads and moves every colour edge
        /// half a chroma pixel, and the reference decoder path this has to match does not do it.
        __device__ __forceinline__ void nv12_sample_rgb(const Nv12View& view, int y, int x,
                                                        float* rgb) {
            const float luma = static_cast<float>(view.y[y * view.y_stride + x]) - 16.f;
            const unsigned char* pair = view.uv + (y >> 1) * view.uv_stride + ((x >> 1) << 1);
            const float u = static_cast<float>(pair[0]) - 128.f;
            const float v = static_cast<float>(pair[1]) - 128.f;
            // Written as three independent expressions rather than a matrix, because these exact
            // coefficients are the contract with the numpy oracle and a reader must be able to
            // compare them line for line.
            rgb[0] = fminf(fmaxf(1.164f * luma + 1.596f * v, 0.f), 255.f);
            rgb[1] = fminf(fmaxf(1.164f * luma - 0.391f * u - 0.813f * v, 0.f), 255.f);
            rgb[2] = fminf(fmaxf(1.164f * luma + 2.018f * u, 0.f), 255.f);
        }

        /// Bilinear sample of the *converted* image, border-clamped. Convention 3.
        ///
        /// The clamping is the BGR path's: the tap *indices* are clamped while the weights keep
        /// using the unclamped floor, which for the coordinate range this kernel produces
        /// (`[-0.5, extent - 1]`) is identical to clamping the coordinate.
        __device__ __forceinline__ void nv12_sample_bilinear(const Nv12View& view, float y, float x,
                                                             float* rgb) {
            const int x0 = static_cast<int>(floorf(x));
            const int y0 = static_cast<int>(floorf(y));
            const int x1 = min(x0 + 1, view.width - 1);
            const int y1 = min(y0 + 1, view.height - 1);
            const int xc = max(x0, 0);
            const int yc = max(y0, 0);
            const float wx = x - static_cast<float>(x0);
            const float wy = y - static_cast<float>(y0);
            ...
            nv12_sample_rgb(view, yc, xc, v00);
            nv12_sample_rgb(view, yc, x1, v01);
            nv12_sample_rgb(view, y1, xc, v10);
            nv12_sample_rgb(view, y1, x1, v11);
            for (int c = 0; c < 3; ++c) {
                rgb[c] = (v00[c] * (1.f - wx) + v01[c] * wx) * (1.f - wy) +
                         (v10[c] * (1.f - wx) + v11[c] * wx) * wy;
            }
        }

        /// One thread per output pixel, gathering from two planes and writing three.
        ///
        /// Structurally the same loop as `letterbox_kernel` — deliberately, so the two can be
        /// read side by side — and it differs only where NV12 differs: the taps come from a
        /// converter instead of a byte fetch, and there is no post-hoc channel index swap
        /// because the converter's output is already RGB.
        __global__ void nv12_letterbox_kernel(const Nv12View* views, int batch,
                                              float* __restrict__ out, int dst_h, int dst_w,
                                              float m0, float m1, float m2, float s0, float s1,
                                              float s2, bool swap_rb, float pad_value) {
            const int total = batch * dst_h * dst_w;
            for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < total;
                 idx += blockDim.x * gridDim.x) {
                const int x = idx % dst_w;
                ...
                float rgb[3];
                if (local_x < 0 || local_y < 0 || local_x >= view.out_w || local_y >= view.out_h) {
                    rgb[0] = rgb[1] = rgb[2] = pad_value;
                } else {
                    const float src_y =
                        (static_cast<float>(local_y) + 0.5f) * view.height / view.out_h - 0.5f;
                    const float src_x =
                        (static_cast<float>(local_x) + 0.5f) * view.width / view.out_w - 0.5f;
                    nv12_sample_bilinear(view, src_y, src_x, rgb);
                }

                // The converter emits RGB, so `swap_rb == true` is the identity here and `false`
                // means "hand me BGR". Inverted relative to the BGR kernel for the same reason the
                // flag exists at all: it names the destination order, not an operation.
                const int c0 = swap_rb ? 0 : 2;
                const int c2 = swap_rb ? 2 : 0;
                dst[0 * plane] = (rgb[c0] - m0) / s0;
                dst[1 * plane] = (rgb[1] - m1) / s1;
                dst[2 * plane] = (rgb[c2] - m2) / s2;
            }
        }
    ...
    void nv12_letterbox_batch(const Nv12View* views_device, int batch, float* out, int dst_h,
        ...
        const int total = batch * dst_h * dst_w;
        const int blocks = std::min(ceil_div(total, kBlockSize), 65535);
        nv12_letterbox_kernel<<<blocks, kBlockSize, 0, stream>>>(
            views_device, batch, out, dst_h, dst_w, params.mean[0], params.mean[1], params.mean[2],
            params.std[0], params.std[1], params.std[2], params.swap_rb,
            static_cast<float>(pad_value));
        check_launch("nv12_letterbox_kernel");
        // No synchronise, for the same reason as `letterbox_batch`: the descriptor table belongs
        // to the caller and outlives the call, so nothing here can be outrun by the kernel.
    }

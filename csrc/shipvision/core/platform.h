...
namespace shipvision {
    ...
    /// A device-side view of one NV12 frame, in the layout a hardware decoder emits.
    ///
    /// Its own struct rather than fields bolted onto `ImageView`, because the two describe
    /// genuinely different memory: `ImageView` is one interleaved plane whose stride is
    /// implicitly `width * 3`, and NV12 is two planes with a stride the *decoder* chooses.
    /// Widening `ImageView` would have put an unused chroma pointer on every BGR call and made
    /// the existing kernel's indexing depend on a field that path never sets.
    ///
    /// **The strides are not optional.** A decoder pads rows to suit itself: GStreamer's
    /// system-memory NV12 gives a 1918-wide frame a 1920-byte stride, and device memory from a
    /// pitched allocator pads 1920 up to 2048. A kernel that assumed `width` would read a
    /// diagonal smear of the frame — plausible-looking output, no error, wrong on exactly the
    /// cameras whose width is not a multiple of the alignment.
    struct Nv12View {
            const unsigned char* y;   ///< luma plane: `height` rows of `y_stride` bytes
            const unsigned char* uv;  ///< chroma plane: `height / 2` rows of `uv_stride` bytes,
                                      ///< U and V interleaved, one pair per 2x2 luma block
            int height;               ///< luma rows; must be even
            int width;                ///< luma columns; must be even
            int y_stride;             ///< bytes per luma row, >= width
            int uv_stride;            ///< bytes per chroma row, >= width
            float scale;              ///< resize factor applied to fit the destination
            int pad_x;                ///< letterbox offset in destination pixels
            int pad_y;
            int out_h;  ///< resized extent inside the destination canvas

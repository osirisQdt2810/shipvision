...
//
// A third translation unit, beside `bindings/tracking.h`, for two reasons. The first is the one
// that file gives: these are host passes with no stream and no device pointer, so interleaving
// them with `module.cpp`'s device work would make its "one GIL transition per entry point" rule
// harder to check by reading. The second is ownership — the single-camera trackers and the
// cross-camera matchers are two families with two lifecycles, and keeping them apart is what
// lets one be worked on without rebuilding the other's opinions.
//

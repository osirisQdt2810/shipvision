...
//
// `csrc/shipvision/` is a plain C++/CUDA library: transformations and kernels that run on a
// CPU or a GPU and have never heard of Python. This file is the only one that knows an
// interpreter exists, and its whole job is marshalling — read numpy buffers, validate, build
// plain PODs, call the library, wrap the result.
//
// NO ENTRY POINT HERE TOUCHES THE GIL, deliberately. A library transforms data; whether a
// call may run concurrently with other Python is the embedding *server's* decision, not
// this library's, and the server is the only party that knows what else is on the thread.
// ShipInfer runs one worker thread per model instance and releases the lock at its own
// boundary. A release in here would be a second, invisible policy underneath that one — and
// a scoped release is not free to get wrong: a `py::` access on the wrong side of it is a
// hard interpreter crash rather than an exception.
//
// What this library owes its embedder instead is that concurrent calls are *safe*: the
// stateful objects — the trackers, the matchers, the `ImageOps` staging ring — say what they
// serialise, so a caller that does release the lock cannot corrupt them.
//
...
namespace {
    ...
    ///
    /// It deliberately holds no py:: object: this is the plain-POD form the kernels read,
    /// and the numpy arrays behind the pointers are kept alive by the caller's argument
    ...
    class ImageOps {
        public:
            ...
            py::tuple nv12_letterbox_into(const std::vector<U8Array>& frames,
                ...
                const auto plans =
                    plan_nv12(frames, widths, dst_h, dst_w, out_bytes, scales.mutable_data(),
                              pads.mutable_data(), extents.mutable_data());
                run_nv12_letterbox(plans, reinterpret_cast<float*>(out_ptr), dst_h, dst_w, params,
            ...
            py::tuple nv12_letterbox_device_into(const py::array_t<int64_t>& descriptors,
                ...
                shipvision::nv12_letterbox_batch(device_views, batch,
                                                 reinterpret_cast<float*>(out_ptr), dst_h, dst_w,
                                                 params, fill, stream);
                slot.record(stream);
                return py::make_tuple(scales, pads, extents);
            }
        ...
        private:
            // ================================================================================
            // The numpy edge. These read Python objects and produce plain PODs.
            // ================================================================================
            ...
            // ================================================================================
            // The library edge. Nothing below touches a py:: object.
            // ================================================================================
...
PYBIND11_MODULE(_C, m) {
    ...
    // they live in their own translation unit — see bindings/mot.h. They are reachable
    ...
    shipvision::bindings::bind_mot(m);
